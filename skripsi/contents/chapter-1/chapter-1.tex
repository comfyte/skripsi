...
% Secara garis besar, tujuan penelitian ini dikategorikan menjadi empat, yaitu persiapan awal (tujuan pertama, kedua, dan ketiga), pengerjaan di sisi Windows Subsystem for Linux (tujuan ke(...)), penghubungan antara sisi WSL dan sisi Windows (tujuan ke(...)), dan pengerjaan di sisi Windows (tujuan ke(...)).
...
\begin{enumerate}
    ...
    % \item Menyelidiki dan menginspeksi cara kerja pemrosesan notifikasi dan pengontrolan media di sistem operasi Linux sungguhan (Ubuntu 22.04 LTS dengan lingkungan \textit{desktop} GNOME) guna diadaptasi kemudian ke lingkungan Windows Subsystem for Linux 2 (WSL2).
    ...
    % \item Mencoba menerapkan hasil yang didapatkan dari tujuan pertama pada instalasi Windows Subsystem for Linux 2 (WSL2) yang belum dimodifikasi (\textit{stock}) dan menandai celah-celah yang ditemukan pada implementasi WSL2 yang mencegah bekerjanya pemrosesan notifikasi dan pengontrolan media layaknya pada sistem operasi Linux sungguhan.
    ...
    % \item Mengimplementasikan komponen-komponen yang diperlukan untuk menutup kekurangan yang ditemukan pada tujuan kedua.

    % \item Menyelidiki metode yang mungkin untuk dapat mengekspos data dari lingkungan Windows Subsystem ke Linux ke lingkungan Windows.

    % \item Mengimplementasikan perangkat lunak (?) di sisi Windows yang mengonsumsi data yang terekspos dari lingkungan Windows Subsystem for Linux (WSL).
    ...
    % TODO: Move the old "too-detailed" points to the flowchart instead (?)
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Objek penelitian:} Penelitian dilakukan pada implementasi Windows Subsystem for Linux saat ini di sebuah perangkat komputer bersistem operasi Windows 11 Pro.
    
    \item \textbf{Metode penelitian:} 
    
    \item \textbf{Waktu dan tempat penelitian:} Penelitian dan percobaan dilakukan dari April 2023 hingga September 2023 di sebuah perangkat komputer di Laboratorium Jaringan Komputer, DTETI. Untuk memudahkan akses, dimungkinkan pengaksesan perangkat komputer secara \textit{remote} dengan menggunakan bantuan perangkat lunak Microsoft Remote Desktop.
    
    \item \textbf{Populasi dan sampel:} Percobaan dilakukan utamanya secara individu oleh penulis sendiri dan dilengkapi oleh dua orang responden yang telah menggunakan Windows Subsystem for Linux sehari-hari.
