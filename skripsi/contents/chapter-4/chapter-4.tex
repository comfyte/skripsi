...
Bus perpesanan D-Bus yang berjalan di dalam WSL secara \textit{default} hanya melakukan \textit{listen} ke alamat soket Unix lokal (disediakan oleh aktivasi soket systemd) saja sehingga belum dapat langsung dihubungi oleh FancyWSL yang berjalan di lingkungan \textit{host} Windows. Agar FancyWSL dapat menghubungi bus perpesanan D-Bus yang berjalan di dalam WSL, perlu dilakukan modifikasi konfigurasi pada sisi WSL mengenai alamat-alamat yang di-\textit{listen} oleh \textit{daemon} D-Bus. Karena penanganan notifikasi dan kontrol media berjalan di bus sesi D-Bus yang berjalan di tiap-tiap sesi pengguna (WSL hanya memiliki satu sesi pengguna saja sehingga mempermudah pengerjaan), modifikasi hanya perlu dilakukan pada konfigurasi bus sesi saja dan FancyWSL hanya perlu menghubungi bus sesi saja. Secara \textit{default}, bus sesi D-Bus memiliki berkas konfigurasi utama yang terletak di \path{/usr/lib/dbus-1/session.conf}. Namun, mengutip dari \cite{}, \textit{daemon} D-Bus memeriksa keberadaan berkas konfigurasi pada sejumlah direktori dan berkas konfigurasi pada suatu direktori dapat menimpa (\textit{override}) atau melengkapi berkas konfigurasi pada direktori lainnya sesuai dengan urutan prioritas direktori-direktori tersebut. Urutan prioritas yang dimaksud adalah sebagai berikut.
\begin{enumerate}
    \item 
\end{enumerate}
