...
├── helpers/
│   ├── __init__.py
│   └── bus_address_parser.py
│   └── constants.py
│   └── obtain_bus_address.py
│   └── platform_verifications.py
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{assets/Screenshot 2024-01-18 123909.png}
...
Pada pengujian ini, didefinisikan dua buah tugas (\textit{task}) yang akan dilakukan oleh partisipan:
\begin{enumerate}
...
Pada partisipan pertama (berinisial H. A. H.), nilai \textit{system usability scale} yang diperoleh yaitu
\[x = (4 + 5 + 3 + 5 + 3) - 5 = 15\]
\[y = 25 - (1 + 1 + 3 + 1 + 1) = 18\]
\[Score = 2,5 * (x + y) = 2,5 * 33 = 82,5\]

Pada partisipan kedua (berinisial K. M.), nilai \textit{system usability scale} yang diperoleh yaitu
\[x = (4 + 5 + 4 + 5 + 3) - 5 = 16\]
\[y = 25 - (1 + 1 + 3 + 2 + 2) = 16\]
