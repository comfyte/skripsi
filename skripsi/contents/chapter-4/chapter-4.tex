...
Sistem operasi Linux memiliki standar penyampaian notifikasi bernama FreeDesktop Desktop Notifications Specification \cite{xdg-desktop-notifications-specification}. Spesifikasi ini mengatur mekanisme penyampaian notifikasi antarklien D-Bus dengan menetapkan antarmuka standar yang harus disetujui oleh klien-klien yang akan berpartisipasi, yakni \verb|org.freedesktop.Notifications| yang terletak pada objek \path{/org/freedesktop/Notifications}. Antarmuka tersebut memiliki sejumlah properti yang perlu menjadi perhatian dalam pengerjaan tugas akhir ini.
\begin{itemize}
    \item \textbf{Properti nama aplikasi (\verb||}\\
    Properti ini berisi informasi nama perangkat lunak atau aplikasi yang mengirimkan notifikasi.
    
    \item \textbf{Properti "replaces ID"}\\
    Priperti ini berisi informasi identitas (ID) notifikasi lain yang telah tertampil untuk digantikan oleh notifikasi yang sedang dikirimkan saat ini.
    
    \item \textbf{Properti \textit{body} konten ()}\\
    \item \textbf{Properti aksi}\\
    \item \textbf{Properti \textit{hints}}\\
    \item \textbf{Properti durasi waktu kadaluarsa}\\
\end{itemize}
...
\begin{table}[h]
    \centering
    \caption{Tabel penghubungan elemen-elemen pengontrolan media di Linux (WSL) dengan Windows}
    \begin{tabularx}{\textwidth}{|c|p{3cm}|X|X|} \hline
        No. & Elemen & Mekanisme di Linux (WSL) & Metode penghubungan dengan Windows\\ \hline
